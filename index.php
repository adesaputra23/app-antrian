...
<head>
    ...
    <!-- Title -->
    <title>Aplikasi Antrian RSUD SUMBAWA</title>
...
<body class="d-flex flex-column h-100" <?= (($url == 'monitor') ? 'style="background-color:' . ($data["warna_background"] ? $data["warna_background"] : "#fffff") : '') . '"' ?>>
    ...
    switch ($url) {
        ...
        case "rekapitulasi":
            ...
            break;
        case "istirahat":
            include 'pages/istirahat/index.php';
            break;
        default:
            include 'main.php';
            break;
    ...
    <a class="btn btn-success mx-4 my-4" href="#" onclick="openInFullscreen()">Istirahat</a>

    <script>
        function openInFullscreen() {
            const newWindow = window.open("index.php?pages=istirahat", "_blank");
        }
    </script>
    ...
    <footer class="footer mt-auto py-4" <?= ($url == 'monitor') ? 'style="height: 5vh;' . 'background-color:' . ($data['warna_accent'] ? $data['warna_accent'] : '#fff') . ';color:' . ($data['warna_text'] ? $data['warna_text'] : '#fff')  . '"' : ''; ?>>
        <div class="container">
            <!-- copyright -->
            <div class="copyright text-center mb-2 mb-md-0">&copy; <?= date('Y') ?> - <a href="https://rsud.sumbawakab.go.id/" target="_blank" class="text-brand text-decoration-none">rsud.sumbawakab.go.id</a>. All rights reserved.
            </div>
        </div>
    </footer>
